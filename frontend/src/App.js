...
const sendRequest = async (data) => {
  ...
  return new Promise((resolve, reject) => {
    canvas.toBlob(async (blob) => {
      ...
      formData.append("file", blob, `request_${id}.png`);

      try {
        await fetch("http://localhost:8080/upload-request", {
          method: "POST",
          body: formData,
        });
        ...
        const pollServer = async () => {
          try {
            const response = await fetch(`http://localhost:8080/get-response/${id}`);
            ...
            if (response.status === 200) {
              ...
              const qrData = await QrScanner.scanImage(imageBitmap, { returnDetailedScanResult: true });

              if (qrData) {
                console.log("Ответ получен:", qrData.data);
                resolve(qrData.data); // Завершаем промис с данными QR
              }
            }
          } catch (err) {
            console.error("Ошибка при запросе:", err);
            reject(err); // В случае ошибки завершаем промис с ошибкой
          }

          // Если ответа нет, ждем 5 секунд и пробуем снова
          setTimeout(pollServer, 5000);
        };

        pollServer(); // Запуск бесконечного опроса

      } catch (error) {
        reject(error); // Ошибка при загрузке файла
      }
    }, "image/png");
  });
};
...
function App() {
  ...
  useEffect(() => {
    sendRequest(JSON.stringify({
      ...
      if (result){
        // Это отвратительно и надо переделать
        if (first_launch) {
          first_launch = false
        }
        else {
        console.log("Ответ ",  typeof result)
        setNote(result)
      }
    }
    else {
